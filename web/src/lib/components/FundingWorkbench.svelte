<script lang="ts">
	...
	let {
        updating = false, onRetry = () => {}, onLoadExample,
		response,
		baseline = null,
	...
	const needsFunding = $derived(response.funding_gap > 0 || response.plans.length > 0);
	const fundingTitle = $derived(response.funding_gap > 0 ? 'Ways to close the reserve gap' : needsFunding ? 'Funding for the selected risk limits' : 'Your modeled reserve is covered');
...
<div class:funding-workbench--preview={isPreview} class="funding-workbench">
	<header class="view-toolbar">
		<div><strong>{source === 'personal' ? 'Funding routes' : 'Synthetic funding routes'}</strong><span>same paths, weights, and reserve policy</span></div>
	...
	<div class="funding-layout">
		<main class="plan-console" aria-busy={updating}>
            {#if updating}<p role="status">Recomputing funding plans for the latest inputs…</p>{:else}
			<section class="plan-header" aria-labelledby="funding-title">
				<div>
					...
					<p>{!needsFunding ? 'Available cash covers the active reserve target. No additional borrowing, sale, or spending deferral is indicated by this forecast.' : deterministic ? 'The schedule is deterministic, so options are ranked by actual cost, feasibility, debt, sale, tax, and deferral fields without a shortfall probability claim.' : 'Each option uses the same modeled draw bundle. Differences reflect funding tradeoffs, not a different market draw.'}</p>
				</div>
				{#if needsFunding}
            ...
            {#if needsFunding && response.funding_evaluation_horizon_days}<p class="solver-note">All funding options are evaluated over {response.funding_evaluation_horizon_days} days, including repayment and settlement.</p>{/if}
            <OptimalPlanPanel {response} {onRetry} {onLoadExample} deterministic={deterministic} />
			<PlanTable
					...
					<ul>
						{#each response.plans as plan (plan.id)}
							<li class:limited={!plan.feasible}>
								...
								<strong>{!plan.feasible ? 'Unavailable' : plan.meets_policy === false ? 'Outside policy' : 'Meets policy'}</strong>
								<p>{!plan.feasible ? (plan.infeasible_reason ?? plan.explanation) : plan.policy_reason ?? 'Within the modeled funding and risk limits.'}</p>
							</li>
						{/each}
					</ul>
			</section>
		...
		{#if response.account_liquidity?.accounts}<AccountLiquidityPanel data={response.account_liquidity} />{/if}
            <p class="solver-note">Named taxable plans use the saved FIFO/HIFO disposal rule. The optimizer selects lots and account types by withdrawal cost. Both use the same tax rates, evaluation horizon, futures and weights.</p>
            {/if}
        </main>
