<script lang="ts">
	...
	const axes = $derived.by(() => {
		...
		return ['X', 'Y', 'Z'].map((label, i) => ({ label, x: 36 + right[i]*22, y: 36 - vertical[i]*22, depth: forward[i], color: ['var(--lab-gizmo-x)', 'var(--lab-gizmo-y)', 'var(--lab-gizmo-z)'][i] })).sort((a, b) => a.depth - b.depth);
	});
</script>
<svg class="orientation-gizmo" viewBox="0 0 72 72" aria-hidden="true">
	<circle cx="36" cy="36" r="26" fill="none" stroke="var(--lab-gizmo-rule)" stroke-opacity=".25" stroke-dasharray="1 4"/>
	{#each axes as axis}
	...
	{/each}
	<circle cx="36" cy="36" r="2" fill="var(--lab-gizmo-center)"/>
</svg>
<style>
