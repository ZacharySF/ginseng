...
export interface PrecisionResult {
 ...
 summary: { cash_shortfall_probability: number; numerical_probability_interval: [number,number];
  absolute_error_bound: number; requested_absolute_error: number; confidence: number;
  precision_met: boolean; stop_reason: string; actual_n: number };
}
...
export class NumericalStore<T> {
 ...
 async run(endpoint: string, request: object, action: 'surface' | 'precision') {
  ...
  const result=await requestEngine<T>(endpoint, { method:'POST', headers:{'content-type':'application/json'},
   body:JSON.stringify({...request,options:{action}}),signal:this.#controller.signal },{requiresAuth:true,timeoutMs:60000});
