...
def test_real_forecast_baseline_preview_context():
    from ginseng.finance_models import ScenarioOverrides
    from ginseng.forecast_api import ForecastRequest, forecast_finance
    from ginseng.supabase import AuthenticatedIdentity

    from tests.test_forecast_api import ForecastRepositoryStub, workspace
    ...
    with EvaluationContext() as c:
        response = forecast_finance(
            request,
            AuthenticatedIdentity(user_id="test", access_token="test"),
            ForecastRepositoryStub(state),
        )
        assert response.baseline.result.cash_paths == response.preview.result.cash_paths
        assert c.counters["path_materializations"] == 1
        assert c.counters["cache_hits"] > 0
