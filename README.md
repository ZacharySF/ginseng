...
`python -m ginseng` is equivalent to the installed command. Omit `--out` for simulation/exact JSON on stdout; diagnostics use stderr and failures return nonzero status. `uv run ginseng tui` opens an interactive terminal UI (curses, no extra dependency) for configuring and running `simulate`/`exact`, viewing results, and browsing saved runs under `artifacts/tui/`. `--material-horizon 60 --horizon 30` predeclares a plan whose earlier days remain stable when the visible horizon grows. New bundles reject extensions beyond that plan. `--sampler legacy_mc` exposes the original generator for comparison; existing HTTP callers retain it by default.
